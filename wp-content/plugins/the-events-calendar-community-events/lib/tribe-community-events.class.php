...
if ( !class_exists( 'TribeCommunityEvents' ) ) {
	...
	class TribeCommunityEvents {
		...
		public function disable_comments_on_page(){
			return TribeEventsTemplates::getTemplateHierarchy( 'blank-comments-template', array( 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
		}
		...
		public function addRoutes( $router ) {
			...
			$template_name = ( tribe_get_option('tribeEventsTemplate', 'default') == 'default' ) ? 'page.php' : tribe_get_option('tribeEventsTemplate', 'default');
		...
		public function isTcePage() {
			...
			if ( empty ( $this->tcePageId ) )
				return false;

			return get_the_ID() == $this->tcePageId;
		}
		...
		public function addScriptsAndStyles() {
			...
			wp_enqueue_style( TribeEvents::POSTTYPE . '-community', $stylesheet_url );

			wp_enqueue_script( TribeEvents::POSTTYPE . '-community', $this->pluginUrl . '/resources/tribe-events-community.js', array('jquery'));
		...
		public function doDelete( $tribe_event_id ) {
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'] , 'tribe_community_events_delete' ) && current_user_can('delete_post', $tribe_event_id) ) {
				...
				if ( isset( $event->ID ) ) {
					...
					$this->enqueueOutputMessage( sprintf( __( 'This event (#%s) does not appear to exist.', 'tribe_community_events_delete' ), $tribe_event_id ) );
				}
			} else {
			...
			include TribeEventsTemplates::getTemplateHierarchy( 'delete', array( 'subfolder' => 'modules', 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
			$output .= ob_get_clean();
		...
		public function doEventForm( $id = null ) {
			...
			if ( ( !$this->allowAnonymousSubmissions && !is_user_logged_in() ) || ( $edit && $tribe_event_id && !is_user_logged_in() ) ) {
				...
				$output .= $this->login_form(__( '<a href="/register">Register here</a>', 'tribe-events-community' ));
				return $output;
			}
			...
			if ( $this->allowAnonymousSubmissions || is_user_logged_in() ) {
				...
				if ( ( isset( $_POST[ 'community-event' ] ) && $_POST[ 'community-event' ] ) && check_admin_referer( 'ecp_event_submission' ) ) {
					...
					$_POST['post_type']   = TribeEvents::POSTTYPE;
					...
					if ( $tribe_event_id && !empty( $_POST[ 'post_content' ] ) && !empty( $_POST[ 'post_title' ] ) ) {
						if ( $this->saveEvent( $tribe_event_id ) ) {
							...
							if ( isset( $_POST['recurrence']['type'] ) && $_POST['recurrence']['type'] != 'None' && !TribeEventsRecurrenceMeta::checkRecurrenceAmount( $tribe_event_id ) ) {
								$max_recurrences = ( apply_filters( 'tribe_events_max_recurrences', 199 ) + 1 );
								$this->enqueueOutputMessage( sprintf( __( 'The original event is saved; recurrences not saved. Please reduce the number of recurrences to no more than %s.', 'tribe-events-community' ), $max_recurrences ) );
							}
							...
							$this->enqueueOutputMessage( __( 'There was a problem saving your event, please try again.', 'tribe-events-community' ), 'error' );
						}
						...
						if ( $tribe_event_id ) {
							...
							if ( empty( $_POST[ 'post_content' ] ) && empty( $_POST[ 'post_title' ] ) ) {
								$this->enqueueOutputMessage( __( 'Event title and description are required.', 'tribe-events-community' ), 'error' );
							} elseif ( empty( $_POST[ 'post_content' ] ) ) {
								$this->enqueueOutputMessage( __( 'Event description is required.', 'tribe-events-community' ), 'error' );
							} elseif ( empty( $_POST[ 'post_title' ] ) ) {
								$this->enqueueOutputMessage( __( 'Event title is required.', 'tribe-events-community' ), 'error' );
							} else {
								$this->enqueueOutputMessage( __( 'There was a problem submitting your event, please try again.', 'tribe-events-community' ), 'error' );
							}
				...
				if ( $show_form ) {
					...
					if ( empty($_POST) || $this->messageType == 'error' ) {
						do_action( 'tribe_events_community_form', $tribe_event_id, $event, TribeEventsTemplates::getTemplateHierarchy( 'edit-event', array('namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE) ) );
						// pops up dialog for recurrring events when editing/deleting
						include TribeEventsTemplates::getTemplateHierarchy( 'recurrence-dialog', array( 'subfolder' => 'modules', 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
					} else {
						include TribeEventsTemplates::getTemplateHierarchy( 'header-links', array( 'subfolder' => 'modules', 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE) );
					}
		...
		public function doVenueForm( $tribe_venue_id ) {
			...
			ob_start();
			include TribeEventsTemplates::getTemplateHierarchy( 'edit-venue', array( 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
		...
		public function doOrganizerForm( $tribe_organizer_id ) {
			...
			ob_start();
			include TribeEventsTemplates::getTemplateHierarchy( 'edit-organizer', array( 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
		...
		public function doMyEvents( $page = null ) {
			...
			if ( is_user_logged_in() ) {
				...
				ob_start();
				include TribeEventsTemplates::getTemplateHierarchy( 'event-list', array( 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
				include TribeEventsTemplates::getTemplateHierarchy( 'recurrence-dialog', array( 'subfolder' => 'modules', 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ));
				$output .= ob_get_clean();
		...
		public function createEvent() {
			...
			if( apply_filters( 'tribe_events_community_submission_should_strip_html', true, $user_id, false ) )
				$_POST['post_content'] = wp_kses_post( $_POST['post_content'] );
			if ( apply_filters( 'tribe_events_community_submission_should_strip_shortcodes', true, $user_id, false ) )
				$_POST['post_content'] = strip_shortcodes( $_POST['post_content'] );

			$should_strip_shortcodes = apply_filters( 'tribe_events_community_submission_should_strip_shortcodes', true, $user_id );
		...
		public function saveEvent( $tribe_event_id ) {
			$user_id = is_user_logged_in() ? wp_get_current_user() : false;
			// These filters are a booleans to determine whether to strip bad stuff. The added arguments are the current user's id and the event id (false for new events, obviously).
			if ( apply_filters( 'tribe_events_community_submission_should_strip_html', true, $user_id, $tribe_event_id ) )
				$_POST['post_content'] = wp_kses_post( $_POST['post_content'] );
			if ( apply_filters( 'tribe_events_community_submission_should_strip_shortcodes', true, $user_id, $tribe_event_id ) )
				$_POST['post_content'] = strip_shortcodes( $_POST['post_content'] );
		...
		public function spam_check() {
			$timestamp = empty($_POST['render_timestamp'])?0:intval($_POST['render_timestamp']);
			if ( !empty($_POST['tribe-not-title']) || $timestamp == 0 || time() - $timestamp < 3 ) { // you can't possibly fill out this form in 3 seconds
		...
		public function formCategoryDropdown( $event = null, $currently_selected = array() ) {
			...
			echo '</div>';
			if ( count( $cats ) > 9 ) {
				echo '<span style="display:block;"><a id="show_hidden_categories" href="">' . sprintf( __('Show all categories ( %d )'), count( $cats ) ) . '</a></span>';
			}
		}
		...
		public function loadRecurrenceData( $postId ) {
			...
			include TribeEventsTemplates::getTemplateHierarchy( 'recurrence', array( 'subfolder' => 'modules', 'namespace' => 'community', 'plugin_path' => $this->pluginPath, 'disable_view_check' => TRUE ) );
