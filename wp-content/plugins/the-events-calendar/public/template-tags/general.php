...
if ( class_exists( 'TribeEvents' ) ) {
	 ...
	 * @return string
	 */
	function tribe_event_featured_image( $post_id = null, $size = 'medium' ) {
		if ( is_null( $post_id ) )
			$post_id = get_the_ID();
		$image_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ), $size );
		$featured_image = '';
		if ( !empty( $image_src ) ) {
			$featured_image .= '<div class="tribe-events-event-image" style="float:left; margin-right: 10px"><a href="'. tribe_get_event_link() .'" title="'. get_the_title( $post_id ) .'"><img src="'.  $image_src[0] .'" title="'. get_the_title( $post_id ) .'" /></a></div>';
		}
